...
Use `requests.http` for quick testing. Example workflow:

1. **Login** and get `@token`
2. **Create tag**, save `@tagId`
3. **Create post**, save `@postId`
4. **Create comment** on the post
5. **Delete post** (optional)

Variables in `requests.http`:
...
Use `{{token}}`, `{{postId}}`, and `{{tagId}}` in the requests to dynamically reference values.
...
# 2. Delete a post
curl -X DELETE http://localhost:3000/posts/ef908815-11d3-41aa-a620-b608307d8e0c \
-H "Authorization: Bearer $TOKEN"
```

Other operations (create post, create comment, etc.) follow the same pattern using the token.
